<template>
  <el-card style="height: 820px">
    <div class="body">
      ...
      <div class="seach-form">
        <el-form :inline="true">
          ...
          <el-form-item label="语言名称">
            <el-input size="small" v-model="table.params.language_name" clearable></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <el-button size="small" type="primary" @click="addTags()">添加语言</el-button>
...
import { pageLang, deleteLang } from "@/api/internation";
export default {
  components: {
    TagsModal,
  },
  ...
  data() {
    return {
      columns: [
        ...
        { title: "语言名称", key: 'language_name', align: 'center' ,width: 200},
  ...
  methods: {
    ...
    getList() {
      let data = {
        ...
        language_name: this.table.params.language_name,
      }
      pageLang(data).then(res => {
        console.log(res);
    ...
    deletesing(id) {
      ...
      if (ids.length == 0) return this.$message.warning("请选择要删除的语言")
      this.$Modal.confirm({
        title: "确认删除",
        content: "<p>是否确认删除此语言</p>",
        onOk: () => {
          deleteLang({ ids }).then(res => {
            if (res.code == 0) {
